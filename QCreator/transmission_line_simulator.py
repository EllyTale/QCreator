...
class Capacitor(TLSystemElement):
    ...
    def kinetic_energy(self):
        return np.asarray([[self.C, - self.C],
                           [-self.C, self.C]]) / 2
...
class Inductor(TLSystemElement):
    ...
    def kinetic_energy(self):
        return np.asarray([[0, 0],
                           [0, 0]]) / 2  # quadratic form (phi1 dot, phi2 dot)

    def potential_energy(self):
        return np.asarray([[1 / self.L, - 1 / self.L],
                           [- 1 / self.L, 1 / self.L]]) / 2  # quadratic form (phi1, phi2)
...
class JJunction(TLSystemElement):
    ...
    def potential_energy(self):
        return np.asarray([[self.E_J, - self.E_J],
                           [- self.E_J, self.E_J]]) / 2

    def nonlinearity(self):
        energy_potential = np.asarray([[self.E_J, - self.E_J],
                                       [- self.E_J, self.E_J]]) / 2

        return ((2 * e / hbar) ** 4) * np.kron(energy_potential, energy_potential) / 24
